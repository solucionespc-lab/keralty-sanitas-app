...
import { optList } from '../constantes/Constantes';
...
const SelectObject = ({
  ...
  const search = (word: string) => {
    const newList = optionsArray.filter((option) => {
      const optionNormal = option?.[target]
        ?.normalize('NFD')
        ?.replace(/[\u0300-\u036f]/g, '')
        ?.toLowerCase();
      ...
      const wordNormal = word
        ...
        ?.replace(/[\u0300-\u036f]/g, '')
        ?.toLowerCase();
      // return option[target].includes(word);
      return optionNormal.includes(wordNormal);
    });
    setOptionList({ options: newList, searchOption: word });
  };
  ...
  const selectedOption = (option: number) => {
    ...
    setOptionList((prev) => ({
      ...prev,
      searchOption: optSelected[target],
    }));
  };
