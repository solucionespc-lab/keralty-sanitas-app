...
import { SelectMultiObjectProps } from '../types/SelectMultiTypes';
import { StateSelectType, keyBoardType } from '../types/SelectTypes';

const SelectMultObject = ({
  label,
  name,
  style,
  value,
  ...
  widthLabel,
  tarjet,
}: SelectMultiObjectProps) => {
  const [optionList, setOptionList] = useState<StateSelectType>({
    options: [],
    searchOption: '',
  });
  ...
  useMemo(() => {
    const listTarjet = value.map((item) => item[tarjet]);
    const newList = optionsArray
      .filter((item) => !listTarjet.includes(item[tarjet]))
      .slice(0, 24);
    const isTarjet = newList.some((item) => item[tarjet]);
    setOptionList({
      options: isTarjet ? newList : [],
      searchOption: '',
    });
  }, [optionsArray, value]);
  ...
  const selectedOption = (option: number) => {
    ...
    const newSelection: any[] = [...value, optSelected];
  ...
  const deleteOption = (option: number) => {
    if (!disabled) {
      const newSelection: any[] = value.filter((_, idx) => idx !== option);
    ...
    <Container>
      ...
      <MultiContainer>
        ...
        <Opciones
          id={name}
          role="combobox"
          style={{ top: 'calc(3rem*1.35)', ...style }}
        >
...
export default SelectMultObject;
